...
# from isegm.utils.visual_attn import visualize_grid_to_grid_with_cls
# from isegm.utils.visual_attn import visualize_grid_to_grid
# import visdom
...
class ISTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        if cfg.model_ema:
            ...
            self.ema_net = ExponentialMovingAverage(model.module if cfg.distributed else model, device=next(model.parameters()).device, decay=1.0 - alpha)
        ...
        if lr_scheduler is not None:
            if self.model_cfg['use_pcgrad']:
                self.lr_scheduler = lr_scheduler(optimizer=self.optim.optimizer)
            else:
    ...
    def training(self, epoch):
        ...
        tbar = tqdm(self.train_data, file=self.tqdm_out, ncols=100,  ascii=True, position=0, leave=False)\
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.model_cfg['use_pcgrad']:  # 是否使用PCGrad
                losses_logging['overall'] = torch.sum(torch.stack(loss))
            else:
            ...
            if self.is_master:
                ...
                if 'search_param' not in self.model_cfg.keys():
                    tbar.set_description(f'Epoch {epoch}, training loss {train_loss/(i+1):.4f}, ema iou {self.train_metrics[0]._ema_iou:.4f}')
                for metric in self.train_metrics:
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            if self.cfg.distributed:
                batch_data = {k: v.to(self.rank) for k, v in batch_data.items()}
            else:
            ...
            # viz.images(orig_image[:5, :, :, :], win='orig_image', opts={"title": 'orig_image'})
            # viz.images(gt_mask[:5, :, :, :], win='gt_mask', opts={"title": 'gt_mask'})
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    if not validation:
                        ...
                        #for m in self.net.feature_extractor.modules():
                        #        m.eval()
                if self.model_cfg['with_prev_mask'] and self.prev_mask_drop_prob > 0 and last_click_indx is not None:
            ...
            if self.cfg.distributed:
                refine_output = self.net.module.refine(images_focus, points_focus, full_feature, full_logits, bboxes)
            else:
                refine_output = self.net.refine(images_focus, points_focus, full_feature, full_logits, bboxes)

            loss = [] if self.model_cfg['use_pcgrad'] else 0.0
            ...
            loss = self.add_loss('affinity_loss', loss, losses_logging, validation,
                                 lambda: (output['instances'], output['attns'],output['attnW']))

            # Attention Weight loss
            if len(output['attnW']) != 0:
                loss = self.add_loss('attenW_loss', loss, losses_logging, validation,
                                     lambda: (output['attnW'], batch_data['instances']))
