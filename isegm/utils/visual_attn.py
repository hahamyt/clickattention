...
def visualize_grid_to_grid(att_map, grid_index, image, grid_sizes=[64,8], alpha=0.6):
    # if not isinstance(grid_size, tuple):
    #     grid_size = (grid_size, grid_size)
    ...
    grid_image = highlight_grid(image, [grid_index], grid_sizes[1])

    mask = att_map[:, grid_index].reshape(grid_sizes[0], grid_sizes[0])
    mask = Image.fromarray(mask).resize((image.size))
...
def highlight_grid(image, grid_indexes, grid_size):
    # if not isinstance(grid_size, tuple):
    #     grid_size = (grid_size, grid_size)
    ...
    for grid_index in grid_indexes:
        x, y = np.unravel_index(grid_index, (grid_size, grid_size))
