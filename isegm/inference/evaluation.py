...
def evaluate_dataset(dataset, predictor, vis=True, max_clicks=20, save_attnW=True, logs_path='', **kwargs):
    all_ious = []
    if vis or save_attnW:
        save_dir = str(logs_path) + '/num_clicks_' + str(max_clicks) + '/' + dataset.name + '/'
    ...
    for index in tqdm(range(len(dataset))):#, leave=False):
        ...
        _, sample_ious, _ = evaluate_sample(sample.image, sample.gt_mask, sample.init_mask, predictor,max_clicks=max_clicks,
                                            vis=vis, save_dir=save_dir, index=index, **kwargs)
...
def evaluate_sample(image, gt_mask, init_mask, predictor, max_iou_thr,
                    ...
                    vis=True, save_dir=None, index=0, callback=None,
                    progressive_mode=True, **kwargs
                    ):
    ...
    with torch.no_grad():
        ...
        for click_indx in range(max_clicks):
            ...
            pred_probs = predictor.get_prediction(clicker, index=index, click_indx=click_indx,
                                                  pred_thr=pred_thr, save_dir=save_dir, **kwargs)
            ...
            # attn_maps = cache['Attention.forward']
            # cross_attn_maps = cache['CrossAttention.forward']

            # visualize_grid_to_grid(attn_maps[7][0, 0, 0:, 0:], 102, image, grid_sizes=[24, 12])

            # visualize_grid_to_grid(attn_maps[0].sum(axis=1, keepdims=True)[0, 0, 0:, 0:], 102, image,
            #                            grid_sizes=[96, 12])

            # visualize_grid_to_grid(cross_attn_maps[0].sum(axis=1, keepdims=True)[0, 0, 0:, 0:], 102, image, grid_sizes=[96, 12])
            ...
            if callback is not None:
                callback(image, gt_mask, pred_probs, index, click_indx, clicker.clicks_list)
