...
class CocoLvisDataset(ISDataset):
    def __init__(self, dataset_path, split='train',img_split='train2017',stuff_prob=0.0,
        ...
        self.ids = np.load('./isegm/data/datasets/ids.npy', allow_pickle=True)
        connection = MongoClient("localhost", 27017)
        database = connection['COCO_LVIS']
    ...
    def get_sample(self, index) -> DSample:
        sample = self.fs.get(self.ids[index]).read()
        return pickle.loads(memoryview(sample))
...
#     connection = MongoClient("localhost", 27017) #, username="admin", password="123456")
#
#     database = connection['COCO_LVIS']
#
...
#
#     def do_task(index):
#         # print('{}/99354'.format(index))
#         sample = dst.get_sample(index)
#         sample = pickle.dumps(sample)
#         return fs.put(sample)
#
#     a = process_map(do_task, range(0, len(dst.dataset_samples)), max_workers=10, chunksize=100)
#
#     np.save('ids.npy', a)
