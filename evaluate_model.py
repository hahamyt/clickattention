...
def parse_args():
    ...
    parser.add_argument('mode', choices=[ 'CDNet', 'Baseline', 'FocalClick','FocalClickDraw', 'NoBRS', 'RGB-BRS',
                                          'DistMap-BRS','f-BRS-A', 'f-BRS-B', 'f-BRS-C','NoRefine','NoRefineDraw'],
                        help='')
    ...
    group_device.add_argument('--gpus', type=str, default='2',
                              help='ID of used GPU.')
    ...
    parser.add_argument('--save_attnW', action='store_true', default=False)
    parser.add_argument('--vis_path', type=str, default='./experiments/vis_val/',
...
def main():
    ...
    if args.mode == 'FocalClickDraw' or args.mode == 'NoRefineDraw':
        cfg.DAVIS585_PATH = cfg.DAVIS585_vis_PATH
...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    checkpoint_path = args.exp_path + 'checkpoints/'
    checkpoints_list = [Path(utils.find_checkpoint(checkpoint_path, args.checkpoint))]
    logs_path = Path(args.exp_path + 'val/' + checkpoints_list[0].stem)
